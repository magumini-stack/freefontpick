...
def _pair_guide_block(db: Session) -> str:
    ...
    return (
        ...
        "고딕·명조가 붙습니다. 손글씨·귀여운과 디스플레이 제목에는 또박또박 쓴 "
        "손글씨와 둥근 계열까지 본문 후보를 넓힙니다 — 개성 있는 제목을 고른 자리에 "
        "늘 같은 고딕만 붙는 것도 답은 아니기 때문입니다. 다만 흘려 쓴 캘리그라피와 "
        "장식체는 어느 자리에서도 문단에 깔지 않습니다. 작게 쌓으면 글자가 엉겨 "
        "읽히지 않습니다. 네 계열이 각각 어떤 글자인지 아래에 적었습니다.</p>"
...
def _pair_ssr_block(db: Session) -> str:
    ...
    화면이 이미 말하고 있는 것("고른 자리의 틀에 조합을 앉힌다")은 적지
    ...
    return (
        ...
        "<li><b>조합 점수</b>는 굵기 위계 · 글자 결 · 본문 읽기 세 가지를 실측값으로 "
        "잰 값입니다. 취향까지 재지는 못하니, 낮게 나와도 마음에 들면 그게 맞습니다.</li>"
        "</ul>"
        "</section>"
