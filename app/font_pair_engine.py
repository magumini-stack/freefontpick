...
TITLE_TARGET = {
    ...
    # 손글씨·귀여운을 합치면서 0.45(손글씨)와 0.60(귀여운) 사이로 옮겼다.
    # 둥근 귀여운 폰트가 손글씨보다 굵은 편이라, 손글씨 값을 그대로 두면
    # 합친 계열의 절반이 목표에서 멀어진다.
    "hand":    {"d": 0.50},
}
...
def generate(db, shape: str = "", script: str = "ko",
    ...
    나온다 — 고딕·명조 제목이면 고딕·명조에서, 손글씨·귀여운과 디스플레이
    제목이면 손글씨·귀여운 계열까지 열린다.
